...
// otra manera de hacerlo PERO CON RETURN
/*
function generarHTMLCategorias() {
    return baseDeDatos.map(cat => `
        <div class="cursor-pointer" onclick="filtrarProductos('${cat.categoria}')">
            <h2 class="text-center mt-2 font-bold bg-gray-800 py-2 text-white">${cat.categoria}</h2>
        </div>
    `).join('');
...
// Luego puedes utilizar esta función para establecer el contenido en el DOM
const categoriasContainer = document.getElementById('categorias');
categoriasContainer.innerHTML = generarHTMLCategorias();
*/
// otra manera de hacerlo pero con apendchild y createElement 
/* 
function mostrarCategorias() {
    const categoriasContainer = document.getElementById('categorias');
    categoriasContainer.innerHTML = ''; // Limpia el contenedor antes de agregar nuevos elementos

    baseDeDatos.forEach(cat => {
        const div = document.createElement('div');
        div.className = 'cursor-pointer';
        div.onclick = () => filtrarProductos(cat.categoria);

        const h2 = document.createElement('h2');
        h2.className = 'text-center mt-2 font-bold bg-gray-800 py-2 text-white';
